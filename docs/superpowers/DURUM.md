...
**Romantizm adayları (12, NPCDef yapısında — felsefe NPC'leriyle aynı sistem):** ✅ 7/12 — **Elise** (süslü diva), **Daniel** (utangaç profesör/nehir biyoloğu), **Nadia** (bohem), **Cassian** (melankolik fenerci), **Rosa** (güneş-masum/fırın çırağı), **Iris** (alaycı kariyerci/gazeteci), **Sigrid** (sert tomboy/balıkçı). Her biri belirgin, farklı bir sosyal tip (arketip/mizaç haritası spec'te; PROJE-BAGLAM kuralı). Romantizm/sosyal konuşmalar **sosyallik** 🫂 tohumu verir → skill tree'de **sosyallik dalı** (sos_t1 `relationship_gain`, sos_t2 `reputation_bonus`, sos_t3 `salary_reduce`). T3 = flört. **Kadınların 5/6'sı bitti** (kalan: Liv). Kalan 5: Liv (dingin şefaatçi), Bjorn (nazik dev), Kai (kibirli), Elias (hevesli şifacı), Matteo (çapkın). Her biri kullanıcı onayıyla.
