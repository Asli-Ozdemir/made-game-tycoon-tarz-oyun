...
**Romantizm adayları (12, NPCDef yapısında — felsefe NPC'leriyle aynı sistem):** ✅ 4/12 — **Elise** (süslü diva), **Daniel** (utangaç profesör/nehir biyoloğu), **Nadia** (bohem), **Cassian** (melankolik fenerci). Her biri belirgin, farklı bir sosyal tip (arketip/mizaç haritası spec'te; PROJE-BAGLAM kuralı). Romantizm/sosyal konuşmalar **sosyallik** 🫂 tohumu verir → skill tree'de **sosyallik dalı** (sos_t1 `relationship_gain`, sos_t2 `reputation_bonus`, sos_t3 `salary_reduce`). T3 = flört. Kalan 8: Rosa (güneş-masum), Iris (alaycı), Bjorn (nazik dev), Sigrid (sert tomboy), Kai (kibirli), Elias (hevesli şifacı), Liv (dingin şefaatçi), Matteo (çapkın). Her biri kullanıcı onayıyla.
