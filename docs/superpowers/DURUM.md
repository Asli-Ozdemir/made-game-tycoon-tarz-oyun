...
| ~~4~~ | ~~**Oyuncu Romantizm Arkı (C1)**~~ → ✅ motor uygulandı (2026-06-06). `romanceStore`: arkadaş→sevgili(confess)→nişanlı(propose)→evli(marry)→çocuk(haveChild,max2). Kapı: isRomanceCandidate+attractedTo+yetişkin+!B-evli. confess→`player_romance_<npc>` (B koşullu evliliği kapanır), marry→`player_married_<npc>`, haveChild→spawnNpc. characterStore: partnerId/spouseId/childIds. **UI bağlandı (2026-06-07):** DialogueView'de romantizm aksiyonları (itiraf/buluşma/teklif/evlen/çocuk) + jestler (demet $200 / yüzük $5000, gameStore'dan para; Greta/kuyumcu flavor — lokasyon yok). C2 epilog (EpiloguePlayer) arcEnd'de açılıyor. **Hâlâ ertelendi:** itiraf/düğün cutscene içeriği, eş enerji etkisi, spawn-çocuk aging/render, dükkan harita-lokasyonu. | `specs/2026-05-31-oyuncu-romantizm-arki-design.md` | `plans/2026-05-31-oyuncu-romantizm-arki.md` |
