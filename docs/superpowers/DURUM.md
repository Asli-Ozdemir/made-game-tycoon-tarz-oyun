...
**Romantizm adayları (12, NPCDef yapısında — felsefe NPC'leriyle aynı sistem):** ✅ 9/12 — **Elise** (süslü diva), **Daniel** (utangaç profesör/nehir biyoloğu), **Nadia** (bohem), **Cassian** (melankolik fenerci), **Rosa** (güneş-masum/fırın çırağı), **Iris** (alaycı kariyerci/gazeteci), **Sigrid** (sert tomboy/balıkçı), **Liv** (dingin şefaatçi/bahçıvan), **Bjorn** (nazik dev/tamirci). **Kadınlar 6/6 ✅, erkekler 3/6.** Her biri belirgin, farklı bir sosyal tip (arketip/mizaç haritası spec'te; PROJE-BAGLAM kuralı). Romantizm/sosyal konuşmalar **sosyallik** 🫂 tohumu verir → skill tree'de **sosyallik dalı** (sos_t1 `relationship_gain`, sos_t2 `reputation_bonus`, sos_t3 `salary_reduce`). T3 = flört. Kalan 3 (erkek): Kai (kibirli), Elias (hevesli şifacı), Matteo (çapkın). Her biri kullanıcı onayıyla.
